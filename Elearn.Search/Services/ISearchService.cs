namespace Elearn.Search.Services
{
    public interface ISearchService
    {
        ...
        Task<IEnumerable<T>> SearchAsync<T>(string query, string? index = null) where T : class;
        Task DeleteAsync(string id, string? index = null);
        Task<IEnumerable<string>> AutocompleteAsync(string prefix, string? index = null, int size = 10, CancellationToken cancellationToken = default);
    }
}
