        ...
        >
          ...
          {!loading && !error && items.length > 0 && (
            <div className="grid grid-cols-3 gap-3">
              {items.map(cat => (
                <div key={cat.id}>
                  <div className="text-sm font-semibold mb-2 text-gray-800 dark:text-gray-200">{cat.name}</div>
                  <ul className="space-y-1.5 text-sm text-gray-600 dark:text-gray-300">
                    <li><a href={`/courses?categoryId=${cat.id}`} className="hover:underline decoration-gray-400/60 hover:text-gray-900 dark:hover:text-white transition-colors">All {cat.name}</a></li>
